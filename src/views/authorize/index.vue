<template>
  <div class="authorize">
    <a-card title="账号授权">
      <div>
        <a-button type="primary" @click="addTbsq" style="margin-right: 16px;margin-bottom: 10px">授权添加淘宝联盟账号</a-button>
        <a-button type="primary" @click="addPddsq" >授权添加多多进宝账号</a-button>
      </div>
      <div>
        <a-tabs default-active-key="tb" @change="tabChange">
          <a-tab-pane key="tb" tab="淘宝联盟授权">
            
          </a-tab-pane>
          <a-tab-pane key="dd" tab="多多进宝授权">
            
          </a-tab-pane>
          <a-tab-pane key="jd" tab="京东联盟授权" disabled>
        ...
        <a-table
          ...
          :scroll="{ x: 600 }"
        >
          <template slot="PlatformType" slot-scope="row">{{row.PlatformType | PlatformTypeFilter}}</template>
          <template slot="EndTime" slot-scope="row">
            {{row.EndTime}}
            <a :style="'color: ' + (row.EndTimeTips == '已过期' ? '#ff4d4f' : '#faad14' )" href="javascript:;">{{row.EndTimeTips || ''}}</a>
            </template>
